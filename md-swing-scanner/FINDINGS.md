...
Nothing from today beyond the two `live_checkpoint.py` wire-ins (Freshness Score, RVOL@Trigger) is adopted into production — the acceptance-delay/streak-confirmation line of work is explicitly a rejection (delay costs more than the filter buys), the runaway-vs-pullback prediction line is also a rejection (body_atr/close_pos describe path shape, not quality), and the consolidation-days/combined-filter results are promising leads pending the critic's read on the open questions above.
