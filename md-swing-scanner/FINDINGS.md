...
| | n | Win rate | Expectancy | Concentration |
|---|---|---|---|---|
| `require_regime=True` (real production) | 30 | 46.7% | −2.424% | 54.8% |
| `require_regime=False` (observation only) | 96 | 42.7% | −2.040% | 27.0% |

Both negative expectancy, both sub-50% win rate, consistent in direction across both framings. **Zero VCP entries in either framing across the entire 195-ticker population** — these names never once form a clean base under `stage2_trend_template`/`base_pivot`, only ever fire Breakout Continuation.

**Honest read**: this does NOT support the "smaller/thinner names have more swing opportunity" hypothesis suggested by the RQ-48 liquidity-bucket work — if anything it points the other way. But n=30/96 is a modest sample by this project's usual standard (thousands, not dozens) — a real, negative-leaning lead, not a confident rejection. **Population parked, not deleted** — `extended_universe.csv` and its fetched price history (`data_cache/`) are kept as-is for reproducibility, but this is not an active research thread; not wired into the daily scan, not currently being expanded further. Revisit only if a specific new reason comes up (e.g., a much bigger version of this same test, or a different hypothesis specific to this population) — don't treat the RQ-48 liquidity-bucket finding as license to keep pursuing smaller-cap names in general, since this specific attempt didn't bear it out.
