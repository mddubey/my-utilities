...
Nothing from today beyond the two `live_checkpoint.py` wire-ins (Freshness Score, RVOL@Trigger) is adopted into production. Standing rejections as of today: acceptance-delay/streak-confirmation as an entry gate; runaway-vs-pullback prediction as a target; acceptance as an execution-state stop-tightening trigger. Standing "real but not yet actionable" findings: body_atr as a fragility signal (not win-rate, not runaway-shape); velocity as a weak swing-side lead; RVOL@Trigger inconclusive pending a larger sample. Freshness Score remains the only signal validated across every population, every exit regime, and every metric (win rate, magnitude, fragility) tested so far.
