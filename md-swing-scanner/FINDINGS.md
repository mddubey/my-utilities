...
The real population stays adequately sized at every value tested (885-1,140, comfortably above Rule #4's 500-trade minimum — RQ-51's real population, by contrast, collapsed to n=1 then 0 by RSI_MIN=70). Swing expectancy improves monotonically across all three populations as the threshold shortens. Options is cleanly monotonic on Primed and Entry Gate, but noisier on the real population specifically — 9 (+0.627%) actually edges out 5 (+0.604%), though 3 (+0.640%) is still best overall — a real, honest wrinkle, not smoothed over. **This is the first EMA34/RSI/momentum-style threshold lead this session that satisfies Research Integrity Rule #3's spirit as cleanly as this** — real, same-direction movement on a real, adequately-sized live-representative population, not just two large historical averages agreeing with each other. Still explicitly a lead pending critic review, not adopted — posed to the critic in full.
