...
**Not fixed yet** — `option_backtest.py`'s `pick_contract()` has no hard constraint preventing an "itm" pick from landing above spot; it only picks nearest-liquid-to-target. A real fix (e.g. require strike ≤ spot for calls, or fall back to the nearest liquid strike that still satisfies moneyness intent rather than pure nearest-to-target) is a backtest-correctness change, not a new strategy idea — but per the standing rule, flagging it here rather than patching it unilaterally. Recommend fixing this before trusting any future ITM+next-vs-ITM+current comparison, and re-running the DTE×moneyness×expiry heatmap once fixed.
