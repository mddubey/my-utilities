...
**RVOL@Trigger final audit — flips from "inconclusive" to real, options-only.** Larger population (n=522, up from the earlier n=364) and a real decile sweep instead of quartiles. Options win rate climbs 43.4%→69.2% across D1-D8 (7 increases, 2 decreases across 9 steps — essentially monotonic, one likely-noise dip at D9, n=52/decile). Swing stays noise (rank-corr -0.044). Checked for outlier inflation: bottom-30% vs top-30% shows a real win-rate/median lift (47.1%→63.1% win, -0.15%→+0.43% median, not outlier-dependent) but the mean-based lift has 65.5% top-10 concentration — above the 40% danger threshold, tail-driven. **Verdict: real, options-only, promote on the median/win-rate basis, not the mean.**
...
Nothing from today beyond the two `live_checkpoint.py` wire-ins (Freshness Score, RVOL@Trigger fix) is adopted into production. Standing rejections as of today: acceptance-delay/streak-confirmation as an entry gate; runaway-vs-pullback prediction as a target; acceptance as an execution-state stop-tightening trigger. Standing "real but not yet actionable" findings: body_atr as a fragility signal (not win-rate, not runaway-shape); velocity as a weak swing-side lead. RVOL@Trigger is now resolved (real, options-only). Portfolio-constrained ranking is unresolved/inconclusive pending a more robust test. Freshness Score remains the only signal validated across every population, every exit regime, and every metric (win rate, magnitude, fragility) tested so far.
