...
def test_current_stop_level_breakout_cont_uses_structural_floor_before_engage():
    # (2026-09-15) BC's initial stop is now a fixed structural_low - 1xATR floor set
    # at entry, not a chandelier trailing peak_close -- see FINDINGS.md SMA21 thread.
    state = dict(entry_price=100, peak_close=100 * (TRAIL_ENGAGE_PCT - 0.001),  # not yet engaged
                 peak_high=101, structural_low=92, target=None, atr_entry=2.0)
    row = _row(atr14=2.0, ema21=110, sma21=105)
    expected = 92 - STRUCTURAL_STOP_ATR_BUFFER * 2.0
    assert current_stop_level("breakout_cont", state, row) == expected


def test_current_stop_level_breakout_cont_switches_to_sma21_trail_after_engage():
    state = dict(entry_price=100, peak_close=100 * (TRAIL_ENGAGE_PCT + 0.01),  # engaged
                 peak_high=105, structural_low=92, target=None, atr_entry=2.0)
    row = _row(atr14=2.0, ema21=95, sma21=100)
    base_stop = 92 - STRUCTURAL_STOP_ATR_BUFFER * 2.0
    sma21_floor = 100 * (1 - SMA21_TRAIL_BUFFER_PCT)
    assert current_stop_level("breakout_cont", state, row) == max(base_stop, sma21_floor)


def test_current_stop_level_breakout_cont_falls_back_to_ema21_when_sma21_nan():
    state = dict(entry_price=100, peak_close=100 * (TRAIL_ENGAGE_PCT + 0.01),  # engaged
                 peak_high=105, structural_low=92, target=None, atr_entry=2.0)
    row = _row(atr14=2.0, ema21=95, sma21=float("nan"))
    base_stop = 92 - STRUCTURAL_STOP_ATR_BUFFER * 2.0
    assert current_stop_level("breakout_cont", state, row) == max(base_stop, 95)
...
def test_current_stop_level_vcp_switches_to_sma21_trail_after_engage():
    # (2026-09-15) VCP's post-engagement floor is now SMA21-2% too -- transferred from
    # breakout_cont after one clean comparison test (see FINDINGS.md's VCP-transfer note).
    state = dict(entry_price=100, peak_close=100 * (TRAIL_ENGAGE_PCT + 0.01),  # engaged
                 peak_high=105, structural_low=92, target=None)
    row = _row(atr14=2.0, ema21=110, sma21=95)
    sma21_floor = 95 * (1 - SMA21_TRAIL_BUFFER_PCT)
    assert current_stop_level("coiled_spring", state, row) == max(92, sma21_floor)  # never below structural floor
    row_low_sma = _row(atr14=2.0, ema21=110, sma21=80)
    assert current_stop_level("coiled_spring", state, row_low_sma) == 92  # floor wins if sma21 dips under it


def test_current_stop_level_vcp_falls_back_to_ema21_when_sma21_nan():
    state = dict(entry_price=100, peak_close=100 * (TRAIL_ENGAGE_PCT + 0.01),  # engaged
                 peak_high=105, structural_low=92, target=None)
    row = _row(atr14=2.0, ema21=95, sma21=float("nan"))
    assert current_stop_level("coiled_spring", state, row) == max(92, 95)


# --- check_exit: stop ---

def test_check_exit_fires_stop_when_close_below_current_stop_level():
    state = dict(entry_price=100, peak_close=110, peak_high=112, structural_low=95, target=None, atr_entry=2.0)
    # engaged -> stop = max(95 - 1*2.0, 105*(1-0.02)) = max(93, 102.9) = 102.9
    row = _row(Close=102.5, Open=103, High=104, Low=102, Volume=1000,
               atr14=2.0, ema21=105, sma21=105, vol_max_run=1000, pp=float("nan"), r1=float("nan"), r2=float("nan"))
...
def test_check_exit_no_exit_when_close_holds_above_stop_and_below_target():
    state = dict(entry_price=100, peak_close=110, peak_high=112, structural_low=95, target=None, atr_entry=2.0)
    # engaged -> stop = max(95 - 1*2.0, 100*(1-0.02)) = max(93, 98) = 98
    row = _row(Close=103, Open=102, High=104, Low=101, Volume=500,
               atr14=2.0, ema21=105, sma21=100, vol_max_run=1000, pp=float("nan"), r1=float("nan"), r2=float("nan"))
    reason, _ = check_exit("breakout_cont", state, row)
...
def test_moving_target_ratchets_up_never_down():
    state = dict(entry_price=100, peak_close=105, peak_high=106, structural_low=90, target=110, atr_entry=1.0)
    # today's fresh pivot level is LOWER than the existing target
    # (stop = max(90-1, 95*0.98) = max(89, 93.1) = 93.1, well below Close=103, no stop fire)
    row_lower = _row(Close=103, Open=102, High=104, Low=101, Volume=500,
                      atr14=1.0, ema21=100, sma21=95, vol_max_run=1000, pp=105, r1=float("nan"), r2=float("nan"))
    ...
    row_higher = _row(Close=103, Open=102, High=104, Low=101, Volume=500,
                       atr14=1.0, ema21=100, sma21=95, vol_max_run=1000, pp=115, r1=float("nan"), r2=float("nan"))
...
def test_check_exit_fires_resistance_when_close_reaches_target():
    ...
    row = _row(Close=111, Open=109, High=112, Low=108, Volume=500,
               atr14=1.0, ema21=100, sma21=95, vol_max_run=1000, pp=float("nan"), r1=float("nan"), r2=float("nan"))
    reason, _ = check_exit("breakout_cont", state, row)
...
def test_climax_does_not_fire_before_min_gain_threshold():
    ...
    state = dict(entry_price=100, peak_close=101, peak_high=101, structural_low=95, target=None, atr_entry=1.0)
    row = _row(Close=102.1, Open=100, High=105, Low=102, Volume=10000,
               atr14=1.0, ema21=100, sma21=100, vol_max_run=10000, pp=float("nan"), r1=float("nan"), r2=float("nan"))
    reason, _ = check_exit("breakout_cont", state, row)
...
def test_climax_fires_when_extended_and_volume_climax_and_weak_close():
    ...
    state = dict(entry_price=entry_price, peak_close=entry_price * CLIMAX_MIN_GAIN_PCT,
                 peak_high=entry_price * CLIMAX_MIN_GAIN_PCT, structural_low=95, target=None, atr_entry=1.0)
    ...
    row = _row(Close=131, Open=130, High=140, Low=129, Volume=50000,
               atr14=1.0, ema21=100, sma21=100, vol_max_run=50000, pp=float("nan"), r1=float("nan"), r2=float("nan"))
...
def test_corp_action_day_right_censors_open_position(monkeypatch):
    ...
    df = pd.DataFrame({
        ...
        "sma21": [100.0, 100.0, 100.0, 100.0, 100.0],
